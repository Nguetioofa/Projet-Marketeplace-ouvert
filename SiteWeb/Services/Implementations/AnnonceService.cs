...
namespace SiteWeb.Services.Implementations
{
    public class AnnonceService : IAnnonceService
    {
        ...
        {
            var response = await _client.PutAsJsonAsync<Annonce>(string.Format("{0}/{1}", _configuration.ApiUrl, ControllerName), annonce);
            return response.IsSuccessStatusCode;

        }
        public async Task<bool> AddAnnonce(Annonce annonce)
        {
            var response = await _client.PostAsJsonAsync<Annonce>(string.Format("{0}/{1}", _configuration.ApiUrl, ControllerName), annonce);
