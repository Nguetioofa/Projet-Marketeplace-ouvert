...
namespace SiteWeb.Services.Implementations
{
    public class AbonnementService : IAbonnementService
    {
        ...
        {
            var response = await _client.PutAsJsonAsync<Abonnement>(string.Format("{0}/{1}", _configuration.ApiUrl, ControllerName), abonnement);
            return response.IsSuccessStatusCode;

        }
        public async Task<bool> AddAbonnement(Abonnement abonnement)
        {
            var response = await _client.PostAsJsonAsync<Abonnement>(string.Format("{0}/{1}", _configuration.ApiUrl, ControllerName), abonnement);
